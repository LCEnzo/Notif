...
void main() {
  ...
  group('an unreachable backend is not a logout', () {
    test(
      'a refused connection keeps the session and stays retryable',
      () async {
  ...
  group('recovery after a failed restore', () {
    test(
      'retries on a backoff and signs in once the backend returns',
      () async {
        SharedPreferences.setMockInitialValues({_rememberedSessionKey: true});
        final settings = await loadedSettings();
    ...
    test(
      'the health probe holds retries back while the backend is down',
      () async {
        SharedPreferences.setMockInitialValues({_rememberedSessionKey: true});
        final settings = await loadedSettings();
        final adapter = install((options) {
  ...
  group('missing session versus rejected session', () {
    ...
    test('a rejected remembered session expires and clears the flag', () async {
      SharedPreferences.setMockInitialValues({_rememberedSessionKey: true});
      final settings = await loadedSettings();
      install(
