...
class _AccountPageState extends State<AccountPage> {
	...
	Future<void> _saveProfile() async {
		...
		try {
			...
			expectSuccessStatus(response, 'Update profile', successCodes: const {200});
	...
	Future<void> _changePassword() async {
		...
		try {
			...
			expectSuccessStatus(response, 'Change password', successCodes: const {200});
		...
		final confirmed = await showDialog<bool>(
			...
			builder: (ctx) {
				...
				return AlertDialog(
					...
					title: Text('Delete account?', style: text$.heading.copyWith(color: tokens.ink)),
					content: Text(
					...
					actions: [
						TextButton(
							...
							child: Text('Cancel', style: text$.body.copyWith(color: tokens.inkMute)),
						),
						TextButton(
									...
									children: [
										...
										NotifCard(
											...
											child: Column(
												crossAxisAlignment: CrossAxisAlignment.start,
												children: [
													_FieldLabel(text$: text$, tokens: tokens, label: 'Username'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _usernameController, hint: 'your-username', tokens: tokens, text$: text$),
													const SizedBox(height: 16),
													_FieldLabel(text$: text$, tokens: tokens, label: 'Email'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _emailController, hint: 'you@example.com', tokens: tokens, text$: text$, keyboardType: TextInputType.emailAddress),
													const SizedBox(height: 16),
													_FieldLabel(text$: text$, tokens: tokens, label: 'Display name'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _nameController, hint: 'Your Name', tokens: tokens, text$: text$),
													if (_profileError != null) ...[
														const SizedBox(height: 12),
														Text(_profileError!, style: text$.micro.copyWith(color: NotifFeedback.error)),
													],
													if (_profileSaved) ...[
														const SizedBox(height: 12),
														Text('Profile saved.', style: text$.micro.copyWith(color: NotifFeedback.success)),
													],
													const SizedBox(height: 16),
													NotifButton(
										...
										NotifCard(
											...
											child: Column(
												crossAxisAlignment: CrossAxisAlignment.start,
												children: [
													_FieldLabel(text$: text$, tokens: tokens, label: 'Current password'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _currentPasswordController, hint: 'Enter current password', tokens: tokens, text$: text$, obscure: true),
													const SizedBox(height: 16),
													_FieldLabel(text$: text$, tokens: tokens, label: 'New password'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _newPasswordController, hint: 'Enter new password', tokens: tokens, text$: text$, obscure: true),
													const SizedBox(height: 16),
													_FieldLabel(text$: text$, tokens: tokens, label: 'Confirm new password'),
													const SizedBox(height: 6),
													_AccountTextField(controller: _confirmPasswordController, hint: 'Confirm new password', tokens: tokens, text$: text$, obscure: true),
													if (_passwordError != null) ...[
														const SizedBox(height: 12),
														Text(_passwordError!, style: text$.micro.copyWith(color: NotifFeedback.error)),
													],
													if (_passwordSaved) ...[
														const SizedBox(height: 12),
														Text('Password changed.', style: text$.micro.copyWith(color: NotifFeedback.success)),
													],
													const SizedBox(height: 16),
													NotifButton(
...
class _FieldLabel extends StatelessWidget {
	...
	const _FieldLabel({required this.text$, required this.tokens, required this.label});

	@override
	Widget build(BuildContext context) {
		return Text(label, style: text$.micro.copyWith(color: tokens.inkMute, letterSpacing: 1.2));
	}
}
