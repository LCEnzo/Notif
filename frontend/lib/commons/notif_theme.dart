...
/// Assemble the full Flutter [ThemeData] from the user's current colorway,
/// scheme, and font-set picks. Single source of truth for the app's visual
...
ThemeData buildNotifTheme({
  ...
  required NotifColorScheme scheme,
  required NotifFontSet fontSet,
}) {
  final tokens = NotifTokens.build(colorway, scheme: scheme);
  final textTheme = NotifTextTheme.forSet(fontSet);
  final materialText = textTheme.toMaterialTextTheme();

  final colorScheme = scheme == NotifColorScheme.dark
      ? ColorScheme.dark(
          surface: tokens.bg1,
