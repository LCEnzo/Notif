...
class EntropyValidator:
	...
	def calculate_password_entropy(self, password: str) -> float:
		...
		has_other = any(c not in string.digits + string.ascii_letters + string.punctuation for c in password)

		categories = [has_digit, has_lower, has_upper, has_punct, has_other]
		lengths = [10, 26, 26, len(string.punctuation), 40]
		char_set = sum(cat * length if cat else 0 for cat, length in zip(categories, lengths, strict=True))
